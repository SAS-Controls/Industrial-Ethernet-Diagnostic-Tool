...
class PollSample:
    ...
    # TCP port probe results
    tcp_success: bool = False
    tcp_time_ms: float = 0.0
    tcp_port: int = 0              # Which port responded
    tcp_error: str = ""
    ...
    def is_reachable(self) -> bool:
        ...
        return self.ping_success or self.cip_success or self.tcp_success

    @property
    def best_response_ms(self) -> float:
        """Best response time from any method."""
        times = []
...
class DeviceMonitor:
    ...
    def __init__(
        ...
        # Default ports: Industrial + common consumer/networking
        # Industrial: HTTP, EtherNet/IP, Modbus, HTTPS, SSH, S7
        # Consumer: RDP, SMB, mDNS, UPnP, alt-HTTP, MQTT
        self.tcp_ports = tcp_ports or [
            80, 443, 22,                  # HTTP, HTTPS, SSH (universal)
            44818, 502, 102,              # EtherNet/IP, Modbus, S7
            3389, 445, 139,               # RDP, SMB (Windows/NAS)
            8080, 8443,                   # Alt HTTP/HTTPS
            23, 161,                      # Telnet, SNMP (switches)
            554, 1883,                    # RTSP (cameras), MQTT (IoT)
        ]
    ...
    def _do_poll(self) -> PollSample:
        """
        Execute one poll cycle — all probes run CONCURRENTLY.

        Old behavior: sequential ping (2s) → TCP (6s) → CIP (3s) = 11s per cycle.
        New behavior: all probes fire in parallel, total time = longest timeout (~3s).

        This is critical for non-CIP devices where CIP always times out —
        the 3-second CIP timeout was blocking the entire poll cycle and making
        the device appear offline even when ping was succeeding.
        """
        now = datetime.now()
        ...
        # Run all enabled probes in parallel threads
        threads = []
        ...
        def do_tcp():
            try:
                success, rtt, port = self._tcp_probe(
                    self.target_ip, self.tcp_ports, self.tcp_timeout)
                sample.tcp_success = success
                sample.tcp_time_ms = rtt
                sample.tcp_port = port
                if not success:
                    sample.tcp_error = "No open ports"
            except Exception as e:
                sample.tcp_success = False
                sample.tcp_error = str(e)
        ...
        if self.enable_cip:
            t = threading.Thread(target=do_cip, daemon=True)
            threads.append(t)
            t.start()

        # Wait for all probes to finish (max wait = longest timeout + buffer)
        max_wait = max(self.ping_timeout, self.tcp_timeout * 2, self.cip_timeout) + 1
        for t in threads:
    ...
    def _tcp_probe(
        ...
        """
        Check if any TCP port is open on the target — all ports in parallel.
        Works for ANY network device — doesn't require CIP/industrial protocols.
        Returns (success, response_time_ms, port_number).
        """
        ...
        def probe_port(port):
            try:
                ...
                if code == 0:
                    with result_lock:
                        # Keep the fastest responding port
                        if not result[0] or elapsed < result[1]:
                            result[0] = True
                            result[1] = round(elapsed, 2)
                            result[2] = port
            except Exception:
                try:
                    sock.close()
        ...
        with ThreadPoolExecutor(max_workers=len(ports)) as ex:
            futs = [ex.submit(probe_port, p) for p in ports]
            for f in as_completed(futs):
                try:
                    f.result(timeout=timeout + 0.5)
                except Exception:
                    pass
        ...
        try:
            ...
            with open(filepath, "w", newline="", encoding="utf-8") as f:
                ...
                writer.writerow([
                    ...
                    "TCP_Success", "TCP_Time_ms", "TCP_Port", "TCP_Error",
                ...
                for s in samples:
                    writer.writerow([
                        ...
                        int(s.tcp_success), f"{s.tcp_time_ms:.2f}", s.tcp_port, s.tcp_error,
